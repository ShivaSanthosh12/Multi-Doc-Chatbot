...
def create_conversational_chain(vector_store):
    ...
    general_system_template = r""" 
                  Use only the following pieces of context from the documents uploaded to answer the question. Do not use any other source apart from the provided context from the documents uploaded. If you don't know the answer, just say that you don't know, don't try to make up an answer. Use twenty sentences maximum. Speak in a formal tone.Keep the answer as concise as possible. Always say "thanks for asking!" at the end of the answer. 
                    {context}
                    """
    ...
    #llm = Ollama(model="llama2",temperature=0.9,callback_manager=AsyncCallbackManager([StreamingStdOutCallbackHandler()]))
    os.environ['REPLICATE_API_TOKEN']="r8_MyLCIi2Bxc926ni8MQm8VLIrVIUM5vZ4fDJt5"
    os.environ['COHERE_API_KEY']="hTv6To5znRBU9FvcVgkza9CgDF9S3IAR82bbdXnF"
    llm = Replicate(
        streaming = True,
...
def main():
    ...
    #     # if loader:
    #     #     text.extend(loader.load())
    st.title("Multi-Doc ChatBot using Zephyr :books:")
